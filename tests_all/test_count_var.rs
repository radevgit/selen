...
#[test]
fn count_with_constant_target() {
    let mut m = Model::default();
    let vars = vec![m.int(1, 5), m.int(1, 5), m.int(1, 5)];
    let target = m.int(3, 3);  // Fixed constant value 3
    let count = m.int(0, 3);
...
#[test]
fn count_dynamic_domain_overlap() {
    let mut m = Model::default();
