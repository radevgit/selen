...
fn main() {
    ...
    model.float_lin_le(&vec![-1.0], &vec![milk], 0.0);
    model.float_lin_le(&vec![-1.0], &vec![milksq], 0.0);
    model.float_lin_le(&vec![-1.0], &vec![butt], 0.0);
    model.float_lin_le(&vec![-1.0], &vec![buttsq], 0.0);
    model.float_lin_le(&vec![-1.0], &vec![cha], 0.0);
    model.float_lin_le(&vec![-1.0], &vec![chasq], 0.0);
    model.float_lin_le(&vec![-1.0], &vec![chb], 0.0);
    model.float_lin_le(&vec![-1.0], &vec![chbsq], 0.0);
    model.float_lin_le(&vec![-1.0], &vec![xm], 0.0);
    model.float_lin_le(&vec![-1.0], &vec![xb], 0.0);
    // ... 232 more constraints omitted for brevity ...
