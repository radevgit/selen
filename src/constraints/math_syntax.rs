...
impl crate::variables::views::ViewRaw for TypedConstant {
    fn get_underlying_var_raw(self) -> Option<crate::variables::VarId> {
        None // Constants don't have underlying variables
    }

    fn min_raw(self, _vars: &crate::variables::Vars) -> crate::variables::Val {
        self.to_val() // Constants have fixed values
    }

    fn max_raw(self, _vars: &crate::variables::Vars) -> crate::variables::Val {
        self.to_val() // Constants have fixed values
    }
}

impl crate::variables::views::View for TypedConstant {
    fn result_type(self, _ctx: &crate::variables::views::Context) -> crate::variables::views::ViewType {
        match self {
            TypedConstant::Integer(_) => crate::variables::views::ViewType::Integer,
            TypedConstant::Float(_) => crate::variables::views::ViewType::Float,
        }
    }

    fn try_set_min(self, min: crate::variables::Val, _ctx: &mut crate::variables::views::Context) -> Option<crate::variables::Val> {
        let self_val = self.to_val();
        if min <= self_val { Some(self_val) } else { None }
    }

    fn try_set_max(self, max: crate::variables::Val, _ctx: &mut crate::variables::views::Context) -> Option<crate::variables::Val> {
        let self_val = self.to_val();
