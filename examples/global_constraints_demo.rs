...
fn main() {
    ...
    println!("📝 Example 6: Global Cardinality Constraint");
    {
        ...
        model.post(counts[0].eq(2)); // Exactly 2 ones
        model.post(counts[1].eq(3)); // Exactly 3 twos  
        model.post(counts[2].eq(2)); // Exactly 2 threes
        model.post(counts[3].eq(1)); // Exactly 1 four
