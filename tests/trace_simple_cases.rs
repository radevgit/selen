...
fn trace_simple_cases() {
    ...
    model.less_than_or_equals(x, float(5.5)); // Use <= instead of <
    ...
    model2.less_than_or_equals(x2, float(5.5)); // Use <= instead of <
    ...
    model3.less_than(x3, float(5.5)); // Use < (this creates Next view)
